    ...
    listEl.innerHTML = drafts.map(d => `
      <div class="draft-item" data-draft-id="${escA(d.draft_id)}" data-emp-id="${escA(d.employee_id)}">
        ...
        <div class="draft-item-actions">
          <button class="draft-action-btn draft-action-edit" data-action="edit-draft" title="Edit in Word">
            ${ICON_SVG.edit}
          </button>
...
function wireDraftActions(listEl, empId) {
  ...
  listEl.querySelectorAll('[data-action="edit-draft"]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const item = btn.closest('.draft-item');
      const draftId = item.dataset.draftId;
      const empIdVal = item.dataset.empId;
      const title = item.querySelector('.draft-item-title')?.textContent || draftId;
      const metaText = item.querySelector('.draft-item-meta').textContent;
      const versionMatch = metaText.match(/v(\d+)/);
      ...
      try {
        ...
        showToast(`Error opening Word: ${err}`, 'error');
        btn.classList.remove('is-editing');
        btn.innerHTML = ICON_SVG.edit;
        btn.style.color = '';
      }
...
// Global Tauri Listeners for background native sync events
if (window.__TAURI__) {
  const { listen } = window.__TAURI__.event;
  
  listen('upload-complete', (event) => {
    console.log("Upload complete", event.payload);
    showToast(`Draft auto-saved to cloud!`, 'success');
  });
  
  listen('conflict', (event) => {
    alert(`Conflict Error: ${event.payload.message}`);
  });
  
  listen('edit-session-ended', (event) => {
    console.log("Session ended for", event.payload);
    const draftId = event.payload.draft_id;
    const btn = document.querySelector(`.draft-item[data-draft-id="${draftId}"] [data-action="edit-draft"]`);
    if (btn) {
      btn.classList.remove('is-editing');
      btn.innerHTML = ICON_SVG.edit;
      btn.style.color = '';
    }
    // Refresh drafts list by simulating click on current vault
    const backBtn = document.getElementById('vault-back-profile');
    if (backBtn) backBtn.click(); // Quick hack: back to profile, wait, actually we want to just reload drafts.
    // Ideally we'd call loadDrafts(empId) but it's bound. We can just dispatch a custom event.
    window.dispatchEvent(new CustomEvent('drafts-changed'));
  });

  window.addEventListener('drafts-changed', () => {
    const currentEmpId = document.querySelector('.draft-item')?.dataset.empId;
    if (currentEmpId) loadDrafts(currentEmpId);
  });
}
